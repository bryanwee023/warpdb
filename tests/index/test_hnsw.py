...
class TestAdd:
    def test_first_add_returns_zero(self, index):
        random.seed(0)
        assert index.add(np.ones(DIM, dtype=np.float32)) == 0

    def test_sequential_ids(self, index):
        random.seed(0)
        ids = [index.add(np.ones(DIM, dtype=np.float32)) for _ in range(5)]
        assert ids == [0, 1, 2, 3, 4]

    def test_entry_point_none_before_first_add(self, index):
        assert index._entry_point is None

    def test_entry_point_set_after_first_add(self, index):
        random.seed(0)
        index.add(np.ones(DIM, dtype=np.float32))
        assert index._entry_point == 0

    def test_vector_store_count_matches_adds(self, index):
        random.seed(0)
        for _ in range(7):
            index.add(np.ones(DIM, dtype=np.float32))
        assert index._vector_store.count() == 7

    def test_node_always_in_layer_zero(self, index):
        random.seed(0)
        vid = index.add(np.ones(DIM, dtype=np.float32))
        assert vid in index._graph[0]

    def test_max_layer_non_negative_after_adds(self, index):
        random.seed(0)
        for _ in range(10):
            index.add(np.ones(DIM, dtype=np.float32))
...
class TestSearch:
    ...
    def test_single_vector_is_found(self, index):
        random.seed(0)
        v = np.ones(DIM, dtype=np.float32)
        index.add(v)
    ...
    def test_returns_at_most_k_results(self, index):
        random.seed(0)
        np.random.seed(0)
        for _ in range(20):
    ...
    def test_returns_fewer_results_than_k_when_index_is_small(self, index):
        random.seed(0)
        for _ in range(3):
            index.add(np.random.rand(DIM).astype(np.float32))
    ...
    def test_results_sorted_ascending_by_distance(self, index):
        random.seed(0)
        np.random.seed(0)
        for _ in range(20):
            index.add(np.random.rand(DIM).astype(np.float32))
    ...
    def test_distances_are_non_negative(self, index):
        random.seed(0)
        np.random.seed(0)
        for _ in range(20):
            index.add(np.random.rand(DIM).astype(np.float32))
    ...
    def test_nearest_neighbor_clearly_separated(self, index):
    ...
    def test_exact_query_match_has_zero_distance(self, index):
        random.seed(0)
